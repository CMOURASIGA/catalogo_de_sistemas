...
const CNCLogoIcon = () => (
  <div className="flex items-center justify-center w-8 h-8 rounded bg-blue-600 flex-shrink-0">
    <span className="text-white font-black text-[7px] leading-none text-center">
      CNC<br/>CORP
    </span>
  </div>
);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Explore o ecossistema de soluções da CNC.</p>
        </div>
        <Link 
          to="/novo" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredSistemas.length > 0 ? (
          filteredSistemas.map((sistema) => (
            <div key={sistema.id} className="bg-white rounded-xl shadow-sm border border-slate-200 hover:shadow-lg hover:border-blue-200 transition-all flex flex-col group">
              <div className="p-6 flex-1">
                ...
                <h3 className="text-lg font-bold text-slate-800 mb-2 group-hover:text-blue-600 transition-colors line-clamp-1">{sistema.nome}</h3>
                
                <div className="mb-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold border ${getCriticidadeStyle(sistema.criticidade)}`}>
                    {sistema.criticidade === 'Crítica' || sistema.criticidade === 'Alta' ? <AlertCircle className="w-3 h-3 mr-1"/> : null}
                    CRITICIDADE {sistema.criticidade.toUpperCase()}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {sistema.tecnologias.slice(0, 3).map((tech) => (
                    <span key={tech} className="text-[10px] font-semibold bg-blue-50 text-blue-700 px-2 py-0.5 rounded border border-blue-100">
                      {tech}
                    </span>
                  ))}
                  {sistema.tecnologias.length > 3 && (
                    <span className="text-[10px] text-slate-400 px-1 py-1 font-medium">+{sistema.tecnologias.length - 3} mais</span>
                  )}
                </div>

                <div className="text-[10px] text-slate-400 grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100">
                  <div>
                    <span className="block font-bold text-slate-500 uppercase tracking-tighter">Área</span>
                    {sistema.areaNegocio}
                  </div>
                  <div className="text-right">
                    <span className="block font-bold text-slate-500 uppercase tracking-tighter">Responsável</span>
                    {sistema.donoNegocio}
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 rounded-b-xl flex justify-end">
                ...
                >
                  Ficha Completa <ArrowRight className="w-4 h-4" />
                </Link>
              </div>
            </div>
          ))
          ...
          <div className="col-span-full py-20 text-center text-slate-400 bg-white rounded-xl border-2 border-dashed border-slate-200">
            <p className="text-lg font-medium">Nenhum sistema encontrado para esta busca.</p>
          </div>
        )}
      </div>
    </div>
